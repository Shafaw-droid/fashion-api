...
class PreferenceResource(Resource):
    ...
    def post(self):
        ...
        preference = CustomerStylingPreference(customer_id=args['customer_id'], preferred_style=args['preferred_style'],
                                               preferred_color=args['preferred_color'],
                                               preferred_fit=args['preferred_fit'],
                                               preferred_season=args['preferred_season'],
                                               budget_range=args['budget_range'])
        db.session.add(preference)
        db.session.commit()
        return preference, 201


class PreferenceList(Resource):
    @marshal_with(StylingPreferenceField)
    def get(self, preference_id):
        preference = CustomerStylingPreference.query.filter_by(preference_id=preference_id).first()
        if not preference:
            abort(404, message='Customer styling preference not found')
        return preference, 200

    @marshal_with(StylingPreferenceField)
    def patch(self, preference_id):
        args = preference_args.parse_args()
        preference = CustomerStylingPreference.query.filter_by(preference_id=preference_id).first()
        if not preference:
            abort(404, message='Customer styling preference does not exist')
        preference.customer_id = args['customer_id']
        ...
        db.session.commit()
        return preference, 'Customer styling preference is successfully updated'
